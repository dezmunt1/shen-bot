...
const { correctMessageId, inlineKeyboard, pagination } = require('../../utils/telegram.utils')
const { adminMongoListener, postmeMongoListener } = require('../../DB/mongo/mongoListener')
...
chatIdResourceScene.enter( async ctx => {
  ...
  const message = 'Вы хотите принудительно добавить новый ресурс, введите ID ресурса'
  const post = await ctx.telegram.sendMessage( chatId, message, inlineKeyboard([['Выйти', 'exitScene']]))
...
chatIdResourceScene.on('text', async ctx => {
  try {
    ...
    ctx.scene.leave()
    ctx.scene.enter('passwordRequest', {...ctx.scene.state, chatId: idResource})
  } catch (error) {
    console.log(error)
  }
})
...
passwordRequestResourceScene.enter( async ctx => {
  ...
  const props = editMessageProps(
    ...
    message,
    [
      [['Нет', 'resourceSetPassword:false'], ['Да', 'resourceSetPassword:true']],
      ['Выйти', 'exitScene']
    ]
  )
  ctx.telegram.editMessageText(...props)
...
totalParseResourceScene.enter( ctx => {
  const { dialogChatId, dialogMessageId } = {...ctx.scene.state}
  const message = ' Да - если отпарсить ресурс полностью, Нет - если парсить начиная с настоящего момента'
  const props = editMessageProps(
    [dialogChatId, dialogMessageId],
    message,
    [
      [['Нет', 'totalParseAdmin:false'], ['Да', 'totalParseAdmin:true']],
      ['Выйти', 'exitScene']
    ]
  )
  ctx.telegram.editMessageText(...props)
...
finishResourceScene.enter( ctx => {
  const {totalParsing, password, chatId, dialogChatId, dialogMessageId} = ctx.scene.state
  const props = editMessageProps(
    [dialogChatId, dialogMessageId],
...
delResourcesListScene.enter( async ctx => {
  ...
  if (ctx.scene.state.delete) {
    ...
    const deleteResource = await postmeMongoListener( {chatId: resourceChatId}, 'delete')
    const message = `[server]: "${resourceChatId}" delete operation. ${ deleteResource ? 'Success' : 'Denied'}!`
  ...
  await ctx.reply(message, Markup.inlineKeyboard(resourceList).extra({parse_mode: 'HTML'}))
})
...
deleteSelectResourceScene.enter( ctx => {
  ...
  const props = editMessageProps(
    ...
    [
      [['Нет', 'btnDelResourceAdmin:false'], ['Да', 'btnDelResourceAdmin:true']],
      ['Выйти', 'exitScene']
    ]
  )
  ctx.telegram.editMessageText(...props)
