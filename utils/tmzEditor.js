...
module.exports = (ctx) => {
    ...
    const inputedTmz = ctx.channelPost ? (ctx.channelPost.text.slice(5)) : ctx.match[1];
    const checkRegex = inputedTmz.match(/^(\+|\-)([0-9]|([0-9][0-2])|12)$/g);

    let message = checkRegex === null ? `Ввод <b>${inputedTmz}</b> не корректен` : `Часовой пояс изменен на <b>"${inputedTmz}"</b>`;
